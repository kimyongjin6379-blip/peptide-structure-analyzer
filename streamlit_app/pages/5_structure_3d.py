...
def main():
    ...
    if input_method.startswith("🧪"):
        ...
        if st.button("🔬 펩타이드 생성", key="3d_digest"):
            with st.spinner(f"{selected_product} 분해 시뮬레이션 중..."):
                ...
                st.session_state['3d_digested_peptides'] = unique_peptides
                st.session_state['3d_digested_product'] = selected_product
                st.success(f"✅ {len(unique_peptides)}개 펩타이드 생성됨")

        if '3d_digested_peptides' in st.session_state:
            peptides = st.session_state['3d_digested_peptides']
            product_name = st.session_state.get('3d_digested_product', '?')
