    ...
    if analysis_mode == "Single Sample":
        ...
        with tab1:
            ...
            with col2:
                ...
                # essential_amino_acids is a dict, not a list
                essential_dict = essential_aa['essential_amino_acids']
                sorted_essential = sorted(essential_dict.items(), key=lambda x: x[1], reverse=True)[:5]
                for aa_code, percentage in sorted_essential:
                    st.write(f"**{aa_code}**: {percentage:.2f}%")
