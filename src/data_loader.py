...
class CompositionLoader:
    ...
    def get_sample_display_name(self, sample_id: str) -> str:
        """
        샘플 ID에 대한 표시 이름 반환 (제품명 + ID)

        Args:
            sample_id: 샘플 ID

        Returns:
            표시 이름 (예: "SOY-1 (Sample_01)")
        """
        if self.data is None:
            self.load_data()

        sample = self.get_sample_by_id(sample_id)
        if sample is not None and 'Sample_name' in self.data.columns:
            product_name = sample.get('Sample_name', sample_id)
            return f"{product_name} ({sample_id})"
        return sample_id

    def get_sample_options(self) -> Dict[str, str]:
        """
        샘플 선택 옵션 반환 (표시명 -> ID 매핑)
        ...
        for idx, row in self.data.iterrows():
            ...
            if 'Sample_name' in self.data.columns:
                product_name = row['Sample_name']
                display_name = f"{product_name} ({sample_id})"
            else:
                display_name = sample_id
            options[display_name] = sample_id
    ...
    def get_peptide_composition(self, sample_id: str, normalize: bool = True) -> Dict[str, float]:
        """
        실제 펩타이드 형태 아미노산 조성 반환 (TAA - FAA)

        Args:
            sample_id: 샘플 ID
            normalize: True면 합이 100%가 되도록 정규화
        ...
        # TAA - FAA
        peptide_comp = {}
        for aa, taa_pct in taa_comp.items():
            faa_pct = faa_comp.get(aa, 0.0)
            peptide_pct = max(0, taa_pct - faa_pct)  # 음수 방지
            if peptide_pct > 0:
                peptide_comp[aa] = peptide_pct

        # 정규화 (선택)
        if normalize and peptide_comp:
            total = sum(peptide_comp.values())
            if total > 0:
                peptide_comp = {aa: (pct/total)*100 for aa, pct in peptide_comp.items()}
